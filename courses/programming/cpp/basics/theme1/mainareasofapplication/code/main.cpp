...
int main()
{
	...
	for_each(istream_iterator<string>{ cin }, {}, [&countOfWordsMap](const string& word) { ++countOfWordsMap[word]; });
 
	map<size_t, set<string>, greater<size_t>> wordsHierarchy;
	for_each(countOfWordsMap.begin(), countOfWordsMap.end(), [&wordsHierarchy](const pair<string, size_t>& wordInfo)
